...
const Header = () => {
  ...
  <header className="py-5 w-full bg-sky-950">
   <div className="max-padd-container flexBetween">
    ...
    <h4 className=" shadow-sm text-white text-[16px] sm:text-[18px] px-2 absolute top-5 rounded-full ">HappyMart</h4>
    </Link>
    ...
    <h4 className=" shadow-sm text-white h-28 w-28 px-2 absolute top-5 rounded-full text-[16px] sm:text-[18px] ">HappyMart</h4>
    </Link>


    <div className="flexBetween gap-x-10 xs:gap-x-20">
        {!menuOpened&&(<CgMenu onClick={toggleMenu} className=" text-white xl:hidden cursor-pointer text-[16px] sm:text-[18px]"/>)}
        <div><FaSearch onClick={()=>setShowSearch((prev)=>!prev)}className="text-xl text-white cursor-pointer text-[13.5px] sm:text-[18px]" /></div>
        <Link to ={'/cart'} className="flex relative">
        <PiShoppingBagOpenFill className="  text-white text-[17px] sm:text-[25px]"/>
        ...
        <div className="group relative ">
            ...
            <RiUserHeartFill  className=" text-white text-2xl cursor-pointer text-[16px] sm:text-[25px]"/>
            </div>
            {token&&<>
